...
for ($t=0; $t<$total_tabs; $t++) {
	...
	if (isset($_POST['AWM_categories_subitems_no_'.$t])) {
		if ($_POST['AWM_categories_subitems_no_'.$t]<1) $_POST['AWM_categories_subitems_no_'.$t]=1;
		if ($_POST['AWM_categories_subitems_no_'.$t]>50) $_POST['AWM_categories_subitems_no_'.$t]=50;
	}
	// AWM_menu_name
	$_POST["AWM_menu_name_".$t] = awm_fix_menu_name($_POST["AWM_menu_name_".$t]);
}
// AWM_menu_path
...
$is_yarpp_enabled = in_array('yet-another-related-posts-plugin/yarpp.php', get_option('active_plugins'));

// If neede, update YARPP's auto_display option
//for ($i=0; $i<$total_tabs; $i++) if (get_option('AWM_Related_'.$i)) update_option('yarpp_auto_display', 0);
...
function awm_wp_settings_link($links) {
	$settings_link = '<a href="options-general.php?page=allwebmenus-wordpress-menu-plugin/allwebmenus-wordpress-menu.php">Settings</a>';
	array_unshift($links, $settings_link);
	return $links;
}

$plugin = plugin_basename(__FILE__);
add_filter("plugin_action_links_$plugin", 'awm_wp_settings_link' );
...
function AWM_options_page() {
	global $AWM_ver, $total_tabs, $is_yarpp_enabled;
?>
...
//	echo "<br><br>|".$_POST['theaction']."|<br><br>";
	...
	if ($_POST['theaction']=="set_defaults") {
		...
		awm_update_option_values();
		$str_code = AWM_create_menu_structure(get_option('AWM_selected_tab'));
		$up_path = get_bloginfo('url') . get_option('AWM_menu_path');
		echo <<<STR
		...
		<div style="background-color: #FEFCF5; border: #E6DB55 solid 1px;">
			<table>
				...
				<tr><td style="width: 800px; text-align: center; padding-top: 0px; padding-bottom: 10px;">
					<textarea cols="100" rows="10" id="loginfo" name="loginfo">$str_code</textarea>
				</td></tr>
				...
				<tr><td style="padding-left: 100px; width: 800px; text-align: left; padding-top: 10px; padding-bottom: 10px;">
					...
					<br>- Make sure that you upload all compiled files in the <strong>$up_path</strong> directory on your server
				</td></tr>
			</table>
		</div>
...
?>
	...
	function awm_show_tab(x) {
		for (i=0; i<<?php echo $total_tabs; ?>; i++) {
...
<form method="post" id="theform" name="theform" action="<?php echo $_SERVER["REQUEST_URI"]; ?>">
	<input id="AWM_selected_tab" name="AWM_selected_tab" type="hidden" value="<?=get_option('AWM_selected_tab');?>"/>
	<table>
		<tr><td width="250"><strong>Online location of menu files:</strong></td>
		...
		<tr><td width="250">&nbsp;</td>
		<td class="itemInfo">
			Define a folder relative to the blog's root folder for your menu files (you need to create this folder).
			<br>The above value corresponds to the following path: <?=get_bloginfo('url');?><span id="AWM_the_path"><?=get_option('AWM_menu_path');?></span>
		</td></tr>
	</table>
	<br><br>
...
<?php 
	for ($t=0; $t<$total_tabs; $t++) {
		echo "<div class='awm_tab_header' style='color: #".(get_option('AWM_menu_active_'.$t)?"0099":"9900")."00;' id='AWM_tab_header_$t' onclick='awm_show_tab($t)'>".get_option('AWM_menu_name_'.$t)."</div>";
	}
?>
		</div>
...
<?php 
	for ($t=0; $t<$total_tabs; $t++) {
?>
			<div class='awm_tab_body' id='AWM_tab_body_<?=$t;?>'>
				<div><input id="AWM_menu_active_<?=$t;?>" name="AWM_menu_active_<?=$t;?>" onclick="awm_uncheck(<?=$t;?>);" type="checkbox" value="true" <?php if (get_option('AWM_menu_active_'.$t)) echo "checked='checked'"; ?> /> <strong>Show "<?=get_option('AWM_menu_name_'.$t);?>" in blog</strong>
				&nbsp;&nbsp;&nbsp;<?php if (!get_option('AWM_menu_active_'.$t)) { ?><span id='AWM_unchecked_<?=$t;?>' style='color:#990000;'>Unchecked! (this menu will not appear in your blog)</span><?php } else { ?><span id='AWM_unchecked_<?=$t;?>' style='color:#009900;'>(this menu will appear in your blog)</span><?php } ?></div>
				<div style="padding-left: 19px; margin-top: 13px;"><strong>Menu name: </strong> <input name="AWM_menu_name_<?=$t;?>" type="text" size="30" value="<?php echo get_option('AWM_menu_name_'.$t) ?>"/></div>
				<div style="padding-left: 19px;" class="itemInfo">Please make sure that the "Menu name" value matches the value in the "Compiled Menu Name" property of the AllWebMenus project file (<i>Tools > Project Properties > Folders</i>). <a href="javascript:void(0)" onclick="awm_folder_info('',<?=$t;?>);">show me</a></div>
				<div id="AWM_folder_info_<?=$t;?>" style="margin-top: 20px; display: none; background-color: #FEFCF5; border: #E6DB55 solid 1px;">
					<table>
						<tr><td style="width: 800px; text-align: center; padding-top: 10px; padding-bottom: 10px;"><strong>More info</strong> - <a href="javascript:void(0)" onclick="awm_folder_info('none',<?=$t;?>);">close</a></td></tr>
						<tr><td style="width: 800px; text-align: center; padding-top: 0px; padding-bottom: 10px;">
							<img src="<?=get_bloginfo('url');?>/wp-content/plugins/allwebmenus-wordpress-menu-plugin/more_info.jpg" width="486" height="560" alt="More info" title="More info"/>
						</td></tr>
						<tr><td style="width: 800px; text-align: center; padding-top: 0px; padding-bottom: 10px;">
							<a href="javascript:void(0)" onclick="awm_folder_info('none',<?=$t;?>);">close</a>
					</table>
				</div>
				<br>
				<fieldset class="options">
					...
					<table width="100%" height="auto" style="padding-left: 40px;">
						...
						<tr><td width="230"><input name="AWM_include_home_<?=$t;?>" type="checkbox" value="true" <?php if (get_option('AWM_include_home_'.$t)) echo "checked='checked'"; ?> /> <strong>"Home"</strong></td>
						<td class="itemInfo">A "Home" item that opens the blog's Home Page.</td></tr>

						<tr><td colspan="2">&nbsp;</td></tr>
						<tr><td colspan="2">&nbsp;</td></tr>

						<tr><td width="230"><input name="AWM_pages_<?=$t;?>" type="checkbox" <?php if (get_option('AWM_pages_'.$t)) echo "checked='checked'"; ?> /> <strong>Pages:</strong></td>
						<td><span style="color: #009900">Show all Pages</span> <span style="color: #990000">except the following:</span></td></tr>
						<tr><td width="230">&nbsp;&nbsp;</td>
						<td><input name="AWM_excluded_pages_<?=$t;?>" type="text" size="55" value="<?php echo get_option('AWM_excluded_pages_'.$t) ?>"/></td></tr>
						<tr><td width="230" align="right">&nbsp;</td>
						<td class="itemInfo">Page IDs, separated by commas (their sub-pages will also be excluded). Example: 34, 59, 140</td></tr>
						<tr><td colspan="2">&nbsp;</td></tr>
						<tr><td width="230">&nbsp;&nbsp;</td>
						<td><input name="AWM_pages_ms_<?=$t;?>" value="main" type="radio" <?php if (get_option('AWM_pages_ms_'.$t) == 'main') echo "checked='checked'"; ?> />&nbsp;Show Pages as Main Menu items</td></tr>
						<tr><td width="230">&nbsp;&nbsp;</td>
						<td><input name="AWM_pages_ms_<?=$t;?>" value="sub" type="radio"<?php if (get_option('AWM_pages_ms_'.$t) == 'sub') echo "checked='checked'"; ?> />&nbsp;Show a Main Menu item named <input name="AWM_pages_name_<?=$t;?>" type="text" size="10" value="<?php echo get_option('AWM_pages_name_'.$t) ?>"/>
						and show Pages as its submenu items</td></tr>
						
						<tr><td colspan="2">&nbsp;</td></tr>
						<tr><td colspan="2">&nbsp;</td></tr>
						
						<tr><td width="230"><input name="AWM_posts_<?=$t;?>" type="checkbox" <?php if (get_option('AWM_posts_'.$t)) echo "checked='checked'"; ?> /> <strong>Posts:</strong></td>
						<td><span style="color: #009900">Show the following Posts:</span></i></td></tr>
						<tr><td width="230">&nbsp;&nbsp;</td>
						<td><input name="AWM_posts_ids_<?=$t;?>" type="text" size="55" value="<?php echo get_option('AWM_posts_ids_'.$t) ?>"/></td></tr>
						<tr><td width="230" align="right">&nbsp;</td>
						<td class="itemInfo">Post IDs, separated by commas. Example: 34, 59, 140</td></tr>
						<tr><td colspan="2">&nbsp;</td></tr>
						<tr><td width="230">&nbsp;&nbsp;</td>
						<td><input name="AWM_posts_ms_<?=$t;?>" value="main" type="radio" <?php if (get_option('AWM_posts_ms_'.$t) == 'main') echo "checked='checked'"; ?> />&nbsp;Show Posts as Main Menu items</td></tr>
						<tr><td width="230">&nbsp;&nbsp;</td>
						<td><input name="AWM_posts_ms_<?=$t;?>" value="sub" type="radio"<?php if (get_option('AWM_posts_ms_'.$t) == 'sub') echo "checked='checked'"; ?> />&nbsp;Show a Main Menu item named <input name="AWM_posts_name_<?=$t;?>" type="text" size="10" value="<?php echo get_option('AWM_posts_name_'.$t) ?>"/>
						and show Posts as its submenu items</td></tr>
						
						<tr><td colspan="2">&nbsp;</td></tr>
						<tr><td colspan="2">&nbsp;</td></tr>
						
						<tr><td width="230"><input name="AWM_categories_<?=$t;?>" type="checkbox" <?php if (get_option('AWM_categories_'.$t)) echo "checked='checked'"; ?> /> <strong>Categories:</strong></td>
						<td><span style="color: #009900">Show all Categories</span> <span style="color: #990000">except the following:</span></td></tr>
						<tr><td width="230">&nbsp;&nbsp;</td>
						<td><input name="AWM_excluded_cats_<?=$t;?>" type="text" size="55" value="<?php echo get_option('AWM_excluded_cats_'.$t) ?>"/></td></tr>
						<tr><td width="230" align="right">&nbsp;</td>
						<td class="itemInfo">Category IDs, separated by commas (their sub-categories will also be excluded). Example: 34, 59, 140</td></tr>
						<tr><td width="230">&nbsp;&nbsp;</td>
						<td><input name="AWM_categories_subitems_<?=$t;?>" type="checkbox" <?php if (get_option('AWM_categories_subitems_'.$t)) echo "checked='checked'"; ?> /> Also show (up to) the <input name="AWM_categories_subitems_no_<?=$t;?>" type="text" size="2" value="<?php echo get_option('AWM_categories_subitems_no_'.$t) ?>"/> newest posts of each Category as its submenu items</td></tr>
						<tr><td width="230">&nbsp;&nbsp;</td>
						<td class="itemInfo">Value must be between 1 and 50.</td></tr>
						<tr><td colspan="2">&nbsp;</td></tr>
						<tr><td width="230">&nbsp;&nbsp;</td>
						<td><input name="AWM_categories_ms_<?=$t;?>" value="main" type="radio" <?php if (get_option('AWM_categories_ms_'.$t) == 'main') echo "checked='checked'"; ?> />&nbsp;Show Categories as Main Menu items</td></tr>
						<tr><td width="230">&nbsp;&nbsp;</td>
						<td><input name="AWM_categories_ms_<?=$t;?>" value="sub" type="radio" <?php if (get_option('AWM_categories_ms_'.$t) == 'sub') echo "checked='checked'"; ?> />&nbsp;Show a Main Menu item named <input name="AWM_categories_name_<?=$t;?>" type="text" size="10" value="<?php echo get_option('AWM_categories_name_'.$t) ?>"/>
						...
						<tr><td width="230"><span style="margin-left: 20px;"><strong>Other:</strong></span></td><td>
							<input type="checkbox" name="AWM_hide_future_<?=$t;?>" value="checkbox" <?php if (get_option('AWM_hide_future_'.$t)) echo "checked='checked'"; ?>/> Hide future-dated posts
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" name="AWM_hide_protected_<?=$t;?>" value="checkbox" <?php if (get_option('AWM_hide_protected_'.$t)) echo "checked='checked'"; ?>/> Hide password-protected items
						</td></tr>
					...
					<input type="hidden" id="awm_initial_menu_type_<?=$t;?>" value="<?=get_option('AWM_menu_type_'.$t);?>"/>
					<table width="100%" height="auto" style="padding-left: 40px;">
						<tr><td colspan="2">&nbsp;</td></tr>
						<tr><td colspan="2" id="awm_changed_type_a_<?=$t;?>" class="updated fade" style="display: none;">
						...
						<tr><td colspan="2" id="awm_changed_type_b_<?=$t;?>">&nbsp;</td></tr>
						
						<tr><td width="230" valign="top"><table>
							<tr><td><span style="cursor: pointer;" onclick="select_menu_type('Dynamic',<?=$t;?>);"><strong><input id="AWM_menu_type_<?=$t;?>_Dynamic" name="AWM_menu_type_<?=$t;?>" value="Dynamic" type="radio" <?php if (get_option('AWM_menu_type_'.$t) == 'Dynamic') echo "checked='checked'"; ?> />&nbsp;"Dynamic" Menu Type</strong></span></td></tr>
							<tr><td><span style="cursor: pointer;" onclick="select_menu_type('Mixed',<?=$t;?>);"><strong><input id="AWM_menu_type_<?=$t;?>_Mixed" name="AWM_menu_type_<?=$t;?>" value="Mixed" type="radio" <?php if (get_option('AWM_menu_type_'.$t) == 'Mixed') echo "checked='checked'"; ?> />&nbsp;"Mixed" Menu Type</strong></span></td></tr>
							<tr><td><span style="cursor: pointer;" onclick="select_menu_type('Static',<?=$t;?>);"><strong><input id="AWM_menu_type_<?=$t;?>_Static" name="AWM_menu_type_<?=$t;?>" value="Static" type="radio" <?php if (get_option('AWM_menu_type_'.$t) == 'Static') echo "checked='checked'"; ?> />&nbsp;"Static" Menu Type</strong></span></td></tr>
						</table></td><td valign="top">
							<div class="itemInfo" id="AWM_menu_type_<?=$t;?>_Dynamic_info" <?=get_option('AWM_menu_type_'.$t)=='Dynamic'?'':'style="display:none;"'; ?>>
							...
							<div class="itemInfo" id="AWM_menu_type_<?=$t;?>_Mixed_info" <?=get_option('AWM_menu_type_'.$t)=='Mixed'?'':'style="display:none;"'; ?>>
							...
							<div class="itemInfo" id="AWM_menu_type_<?=$t;?>_Static_info" <?=get_option('AWM_menu_type_'.$t)=='Static'?'':'style="display:none;"'; ?>>
					...
					<table width="100%" height="auto" style="padding-left: 40px;">
						...
						<?php if (!$is_yarpp_enabled) echo "<br><span class='itemInfo' style='color:#990000;'>Currently not installed and activated</span>";?>
						<br><span class="itemInfo"><a href="http://wordpress.org/extend/plugins/yet-another-related-posts-plugin/" target="_blank">get it here</a></span></td>
						<td valign="top"><input type="checkbox" name="AWM_Related_<?=$t;?>" value="checkbox" <?php if (get_option('AWM_Related_'.$t)) echo "checked='checked'"; if (!$is_yarpp_enabled) echo "disabled='disabled'"; ?>/>
						Show a "Related Posts" item when viewing a post</td>
						<tr><td width="230" valign="top">&nbsp;</td>
						<td class="itemInfo">
							<?php if ($is_yarpp_enabled) { ?>
...
?>
	...
	<br>
	<br>
	<div style="text-align: center;" class="itemInfo"><span style="color:#990000;">Note:</span> Always click the "Save settings" button below to apply your changes. If you leave this page without saving you will lose your unsaved changes.</div>
...
function AWM_generate_linking_code() {
	global $total_tabs, $is_yarpp_enabled;
	
	for ($t=0; $t<$total_tabs; $t++) {
		if (!get_option('AWM_menu_active_'.$t)) continue;
		
		$awm_name = get_option('AWM_menu_name_'.$t);
		...
		if (get_option('AWM_menu_type_'.$t)=='Dynamic') {
			AWM_create_dynamic_menu($t,false);
		} elseif (get_option('AWM_menu_type_'.$t)=='Mixed') {
			AWM_create_dynamic_menu($t,true);
		}
		$parentgroup = "wpgroup";
		
		// only if we are viewing a single post
		if ($is_yarpp_enabled && get_option('AWM_Related_'.$t) && is_single()) {
			// convert quotes to add code to item's <Text> property
			$related = related_posts('', false);
			$related = str_replace('"', "'", $related);
			$related = str_replace("'", "\'", $related);
			$related = ereg_replace(chr(10), "", $related);
			$related = ereg_replace(chr(13), "", $related);
			$related = str_replace('\n', '', $related);
			$related = str_replace('\r', '', $related);
			$related = trim($related);
			
			echo "IRP=$parentgroup.newItem('style=mymenu_'+(wplevel==0?'main_item_style':'sub_item_style')+';visible=0');\n";
			...
			echo "IRP2=IRP1.newItem('style=". $awm_name ."_'+(wplevel==0?'sub_item_style':'sub_item_plus_style')+';text0=" . $related . ";htmlMode=1;');\n";
		}
