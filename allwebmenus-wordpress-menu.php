...
class AWM_Plugin
{
    public $optionsMessage,$AWM_ver , $awm_total_tabs, $awm_is_yarpp_enabled, $wpdb, $dataArray,$awm_table_name,$awm_wp_nav_array;
...
http://localhost/wordpress/wp-content/plugins/allwebmenus-wordpress-menu-plugin/menu/
...
?>
	...
	?>
					...
					<table width="100%" height="auto" style="padding-left: 40px;">
						...
						<?php if (!$this->awm_is_yarpp_enabled) echo "<br><span class='awm_itemInfo' style='color:#990000;'>Currently not installed and activated</span>";?>
						...
						<td valign="top"><input type="checkbox" name="AWM_Related_<?php echo $awm_t;?>" value="checkbox" <?php if ($myrows[$awm_t]->related) echo "checked='checked'"; if (!$this->awm_is_yarpp_enabled) echo "disabled='disabled'"; ?>/>
...
function awm_menu_position( $items, $args){
    ...
    if (isset($this->awm_wp_nav_array[$args->theme_location]))
        return $items."<div style='clear:both;'id='".$this->awm_wp_nav_array[$args->theme_location]."'></div>";
    else
        return $items;
}
...
function awm_print_menu_position($args){
   ...
   $myrows =  $this->wpdb->get_results("SELECT name FROM $this->awm_table_name WHERE position LIKE '". $args['theme_location']."' ORDER BY id ASC" );
