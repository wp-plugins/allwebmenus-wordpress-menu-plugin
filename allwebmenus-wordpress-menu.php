...
require_once ('wp-session-manager-master/wp-session-manager.php');
/*
 *class: AWM_Plugin
 * class of plugin
...
function AWM_init_hook(){
	...
	if ($check && !awm_add_code()) {
		...
		$this->optionsMessage =
			"<div class=\"updated fade\" style=\"margin-top: 20px;\"><p><strong>Linking code could not be added automatically. You have to add it by yourself. Open the \"header.php\" file (found at \"".content_url()."/themes/YourSiteTheme\") and add this code <br /><textarea readonly cols=50 rows=4><?php if (function_exists('AWM_generate_linking_code'))\nAWM_generate_linking_code(); ?></textarea><br /> right after the &lt;body&gt; tag.</strong></p>
...
<img alt="javascript menu / css menu builder" height="203" src="<?php echo plugins_url();?>/allwebmenus-wordpress-menu-plugin/awm5snap-wordpress-menu-plugin-info.jpg" style="padding-left: 28px;"width="270" />
<p><a href="http://www.likno.com/allwebmenusinfo.html">Features</a> &nbsp; <a href="http://www.likno.com/download.html">Download</a> &nbsp; <a href="http://www.likno.com/examples.html">Menu Examples</a> &nbsp; <a href="http://www.likno.com/awmstyles.php">Menu Themes</a> &nbsp; <a href="http://www.likno.com/awmregister.php">Purchase</a></p>
		...
		<br><br>
<img alt="blog" height="203" src="<?php echo plugins_url();?>/allwebmenus-wordpress-menu-plugin/v5_addins-wordpress-menu-plugin-info-blog.jpg" width="270"/>
		</td>
	</tr>
</table>
...
content, etc. </em> </p>
		...
		<td><img alt="wordpress javascript menu css menu plugin" height="306" src="<?php echo plugins_url();?>/allwebmenus-wordpress-menu-plugin/v5_addins-wordpress-menu-plugin-info.jpg" width="217" /></td>
	</tr>
</table>
...
<?php
        ...
        echo $wp_session['message']."<br>";
		unset ( $wp_session['message'] );
?>
...
<?php
	if (isset($_GET['generated'])){
		...
		$awm_up_path = site_url() . get_option('AWM_menu_path');
?>
		...
		<div style="background-color: #FEFCF5; border: #E6DB55 solid 1px; padding-left:15px; margin-top: 20px;">
			<table>
			...
			<tr><td style="padding-left: 100px; width: 800px; text-align: left; padding-top: 10px;">
					- Select it and <b>copy</b> it (press <strong>Ctrl+C</strong>) from above
	...
	<table id="uploader">
		...
		<td class="awm_itemInfo">
					Browse for the ZIP file, then click the "Upload ZIP file" button to upload it.<br />
		</td></tr>
	</table>
<br></div>
...
?>
			<div class='awm_tab_body' id='AWM_tab_body_<?php echo $awm_t;?>'>
				...
				<div style="padding-left: 19px; margin-top: 22px;"><strong>Menu name: </strong> <input name="AWM_menu_name_<?php echo $awm_t;?>" id="AWM_menu_name_<?php echo $awm_t;?>" type="text" size="30" value="<?php echo $myrows[$awm_t]->name ?>"/></div>
				...
				<div id="AWM_folder_info_<?php echo $awm_t;?>" style="margin-top: 20px; display: none; background-color: #FEFCF5; border: #E6DB55 solid 1px;">
					<table>
						...
						<tr><td style="width: 800px; text-align: center; padding-top: 0px; padding-bottom: 10px;">
							<img src="<?php echo plugins_url();?>/allwebmenus-wordpress-menu-plugin/more_info.jpg" width="527" height="513" alt="More info" title="More info"/>
						</td></tr>
						<tr><td style="width: 800px; text-align: center; padding-top: 0px; padding-bottom: 10px;">
				...
				<div style="padding-left: 19px;" class="awm_itemInfo">
					...
					<?php echo site_url().get_option("AWM_menu_path")?>
					</span>
					<br>
                    It is created automatically during this action. If your server's settings do not permit this you will have to create this folder yourself (eg: FTP).
                    <br/>
					Info: This is the online folder where the AllWebMenus "compiled menu ZIP file" extracts its contents (menu engine, styles, etc.) every time you upload it through the "Publish Menu" action.
					</div>
				<br>
...
?>
						...
						<tr><td>&nbsp;</td></tr></table>
                                                <div class="AWM_section">Other options for menu items</div>
					...
					<table width="100%" height="auto" style="padding-left: 40px;">
						...
						<tr><td width="230" valign="top"><table>
							...
							<div class="awm_itemInfo" id="AWM_menu_type_<?php echo $awm_t;?>_Dynamic_info" <?php echo $myrows[$awm_t]->type=='Dynamic'?'':'style="display:none;"'; ?>>
								...
								<p><a href="http://www.likno.com/blog/wordpress-javascript-menu/1184/" target="_blank">View a short presentation explaining all your settings</a></p>
							</div>
	...
	?>
                                    ...
                                    <div class="awm_menu_position" id="awm_widget_menu_position_<?php echo $awm_t;?>" <?php if ($myrows[$awm_t]->position !="awm_widget") echo 'style="display: none;"';?>>You have selected the <em>Widget</em> positioning method.<br /><br/>
                                        A widget instance for this menu is initially created and found in the <a href="<?php echo admin_url('widgets.php');?>#wp_inactive_widgets">widgets administration page</a>, under the "Inactive Widgets" section. Just drag this widget instance and drop it inside the Widget Area that you want.
                                        <br/><br/> ***Note: this method applies ONLY when you ALSO use the "Relative to Element" positioning option within your AllWebMenus project (without changing its "Default ID" selection)***</div>
                                    <div class="awm_menu_position" id="awm_theme_menu_position_<?php echo $awm_t;?>" <?php if ($myrows[$awm_t]->position =="awm_widget" || $myrows[$awm_t]->position == '0') echo 'style="display: none;"';?>>
                                        You have selected the <em>Theme Menu Location</em> positioning method.<br /><br/>
...
function AWM_generate_linking_code() {
	...
	for ($awm_t=0; $awm_t<count($myrows); $awm_t++) {
		...
		if ($gnr=="JS" || $gnr=="ULLI") {
			...
			echo "<script charset='UTF-8' src='" . site_url() . get_option('AWM_menu_path') . $awm_name . ".js' type='text/javascript'></script>\n";
			...
			echo "<link href='". site_url() . get_option('AWM_menu_path') .$awm_name.".css' rel='stylesheet' type='text/css' />\n";
			echo "<script charset='UTF-8' src='". site_url() . get_option('AWM_menu_path') .$awm_name.".js' type='text/javascript'></script>\n";
		}
...
//			echo $myrows[$awm_t]->menu_structure;
	...
	if ($filter_flag) {
       echo "<script> jQuery(document).ready( function (){jQuery('div[id*=\'awmAnchor\']').siblings('.genesis-nav-menu').remove();});</script>";  // genesis_markup case
