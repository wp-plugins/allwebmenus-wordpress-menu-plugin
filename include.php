...
function awm_set_first_time_options() {
	...
	for ($t=0; $t<$total_tabs; $t++) {
		...
		add_option('AWM_menu_name_'.$t, 'menu'.($t+1));
...
function awm_set_default_option_values($t) {
	...
	update_option('AWM_menu_name_'.$t, 'menu'.($t+1));
...
function AWM_check()
{
	...
	$the_msg = array();
	$realpath = dirname(__FILE__) .'/../../..'. get_option('AWM_menu_path');
	$path = get_bloginfo('url') . get_option('AWM_menu_path');
	...
	for ($t=0; $t<$total_tabs; $t++) {
		...
		if (! ($menufile = fopen($realpath . $awm_name, 'r'))) {
			$the_msg[$t] = "Menu ".get_option('AWM_menu_name_'.$t)." was not found at: ". $path . $awm_name;
			continue;
		} elseif (! ($mfile = fread($menufile, filesize($realpath . $awm_name)))) {
			$the_msg[$t] = "Could not read menu at: ". $path . $awm_name;
			continue;
		}
		...
		if (function_exists('curl_init')) {
			if (! ($awm_tmp = geturl($params))) {
				$the_msg[$t] = "Could not retrieve version information for ".get_option('AWM_menu_name_'.$t).". Please <a href='mailto:support@likno.com?subject=WordPress: Error while retrieving version info'>contact Likno</a> for more information.";
			} else {
				$the_msg[$t] = $awm_tmp;
			}
			continue;
	...
	$the_full_msg = "";
	if ($has_msg) {
		$the_full_msg = "<div class='updated fade'>";
		for ($t=0; $t<$total_tabs; $t++) if ($the_msg[$t] != "") $the_full_msg .= "<br><strong>Note about ".get_option('AWM_menu_name_'.$t).": </strong><br>".$the_msg[$t]."<br>";
		$the_full_msg .= "<br><input type='button' value='Hide Notifications' onclick='theform.theaction.value=\"hide_msg\"; theform.submit();'/><br>&nbsp;</div>";
	}
