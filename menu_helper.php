...
function AWM_create_dynamic_menu($awm_t, $awm_is_sub) {
	...
	echo "\n";
	if (get_option('AWM_use_custom_menu_'.$awm_t)) { // if user wants a custom menu
		$awm_ic = AWM_create_existing_dynamic_menu($awm_t, $awm_parentgroup, $awm_ic, false);
...
function AWM_create_dynamic_menu__categories_step($awm_t, $awm_ic, $awm_parentgroup, $awm_cats, $awm_depth, $awm_group, $awm_recent, $awm_isXML) {
	...
	for ($awm_i=0; $awm_i<count($awm_cats); $awm_i++) {
		if ($awm_cats[$awm_i]->category_parent==$awm_group) {
			...
			else echo "item".$awm_depth."=".$awm_parentgroup.".newItem('style=".$awm_m."_'+((wplevel+$awm_depth)==0?'main_item_style':((wplevel+$awm_depth)==1?'sub_item_style':'sub_item_plus_style'))+';itemid=".($awm_ic++).";text0=".str_replace("'","\'",$awm_cats[$awm_i]->cat_name).";".(get_category_link($awm_cats[$awm_i]->category_ID)!=""?"url=".get_category_link($awm_cats[$awm_i]->category_ID):"")."');\n";
			if (AWM_cat_has_kids($awm_cats[$awm_i]->category_ID, $awm_cats)) {
				if ($awm_isXML) {
					$awm_xml_out .= AWM_create_dynamic_menu__categories_step($awm_t, $awm_ic, "wpsubMenu".$awm_depth, $awm_cats, $awm_depth+1, $awm_cats[$awm_i]->category_ID, $awm_recent, true);
				} else {
					...
					$awm_ic = AWM_create_dynamic_menu__categories_step($awm_t, $awm_ic, "wpsubMenu".$awm_depth, $awm_cats, $awm_depth+1, $awm_cats[$awm_i]->category_ID, $awm_recent, false);
				}
				...
				if ($awm_recent[$awm_j]->term_id==$awm_cats[$awm_i]->category_ID) {
					if (!$awm_isXML) echo "wpsubMenuRec=item".$awm_depth.".newGroup('style=".$awm_m."_'+((wplevel+$awm_depth)==0?'sub_group_style':'sub_group_plus_style')+((typeof(wphf_".$awm_m.")=='object')?((wplevel+$awm_depth)==0?wphf_".$awm_m."[0]:wphf_".$awm_m."[1]):''));\n";
					while ($awm_j<count($awm_recent) && $awm_recent[$awm_j]->term_id==$awm_cats[$awm_i]->category_ID && $awm_counter++<get_option('AWM_categories_subitems_no_'.$awm_t)) {
						...
						else echo "item".($awm_depth+1)."=wpsubMenuRec.newItem('style=".$awm_m."_'+((wplevel+$awm_depth)==0?'sub_item_style':'sub_item_plus_style')+';itemid=".($awm_ic++).";text0=".str_replace("'","\'",$awm_recent[$awm_j]->post_title).";".(get_permalink($awm_recent[$awm_j]->ID)!=""?"url=".get_permalink($awm_recent[$awm_j]->ID):"")."');\n";
						$awm_j++;
					}
				}
			}
 ...
 */
function AWM_create_existing_dynamic_menu($awm_t, $awm_parentgroup, $awm_ic, $awm_isXML) {
	$awm_depth = 0;
	$awm_m = get_option('AWM_menu_name_'.$awm_t);
	$awm_term_id = get_option('AWM_use_custom_menu_id_'.$awm_t);
	$awm_xml_out = "";
	
	$menu_items = wp_get_nav_menu_items( $awm_term_id );
	
//	print_r($menu_items);
	if ($awm_term_id==-1) return $awm_isXML?"":$awm_ic;
	else return AWM_create_existing_dynamic_menu__step($awm_t, $awm_ic, $awm_parentgroup, $menu_items, $awm_depth, 0, $awm_recent, $awm_isXML);
}
...
function AWM_create_existing_dynamic_menu__step($awm_t, $awm_ic, $awm_parentgroup, $menu_items, $awm_depth, $awm_group, $awm_recent, $awm_isXML) {
	$awm_m = get_option('AWM_menu_name_'.$awm_t);
	$awm_xml_out = "";
	
	for ($awm_i=0; $awm_i<count($menu_items); $awm_i++) {
		if ($menu_items[$awm_i]->menu_item_parent==$awm_group) {
			if ($awm_isXML) $awm_xml_out .= "<item><id>page_".$menu_items[$awm_i]->ID."</id><name>".$menu_items[$awm_i]->title."</name><link>".$menu_items[$awm_i]->url."</link><submenu>";
			else echo "item".$awm_depth."=".$awm_parentgroup.".newItem('style=".$awm_m."_'+((wplevel+$awm_depth)==0?'main_item_style':((wplevel+$awm_depth)==1?'sub_item_style':'sub_item_plus_style'))+';itemid=".($awm_ic++).";text0=".str_replace("'","\'",$menu_items[$awm_i]->title).";".($menu_items[$awm_i]->url!=""?"url=".$menu_items[$awm_i]->url:"")."');\n";
			if (AWM_existing_has_kids($menu_items[$awm_i]->ID, $menu_items)) {
				if ($awm_isXML) {
					$awm_xml_out .= AWM_create_existing_dynamic_menu__step($awm_t, $awm_ic, "wpsubMenu".$awm_depth, $menu_items, $awm_depth+1, $menu_items[$awm_i]->ID, $awm_recent, true);
				} else {
					echo "wpsubMenu".$awm_depth."=item".$awm_depth.".newGroup('style=".$awm_m."_'+((wplevel+$awm_depth)==0?'sub_group_style':'sub_group_plus_style')+((typeof(wphf_".$awm_m.")=='object')?((wplevel+$awm_depth)==0?wphf_".$awm_m."[0]:wphf_".$awm_m."[1]):''));\n";
					$awm_ic = AWM_create_existing_dynamic_menu__step($awm_t, $awm_ic, "wpsubMenu".$awm_depth, $menu_items, $awm_depth+1, $menu_items[$awm_i]->ID, $awm_recent, false);
				}
			}
			if ($awm_isXML) $awm_xml_out .= "</submenu></item>";
		}
...
function AWM_create_dynamic_menu__pages_step($awm_t, $awm_ic, $awm_parentgroup, $awm_pages, $awm_depth, $awm_group, $awm_recent, $awm_isXML) {
	...
	for ($awm_i=0; $awm_i<count($awm_pages); $awm_i++) {
		if ($awm_pages[$awm_i]->post_parent==$awm_group) {
			...
			else echo "item".$awm_depth."=".$awm_parentgroup.".newItem('style=".$awm_m."_'+((wplevel+$awm_depth)==0?'main_item_style':((wplevel+$awm_depth)==1?'sub_item_style':'sub_item_plus_style'))+';itemid=".($awm_ic++).";text0=".str_replace("'","\'",$awm_pages[$awm_i]->post_title).";".(get_permalink($awm_pages[$awm_i]->ID)!=""?"url=".get_permalink($awm_pages[$awm_i]->ID):"")."');\n";
			if (AWM_page_has_kids($awm_pages[$awm_i]->ID, $awm_pages)) {
				if ($awm_isXML) {
					$awm_xml_out .= AWM_create_dynamic_menu__pages_step($awm_t, $awm_ic, "wpsubMenu".$awm_depth, $awm_pages, $awm_depth+1, $awm_pages[$awm_i]->ID, $awm_recent, true);
				} else {
					...
					$awm_ic = AWM_create_dynamic_menu__pages_step($awm_t, $awm_ic, "wpsubMenu".$awm_depth, $awm_pages, $awm_depth+1, $awm_pages[$awm_i]->ID, $awm_recent, false);
				}
			}
			if ($awm_isXML) $awm_xml_out .= "</submenu></item>";
		}
...
function AWM_create_menu_structure($awm_t) {
	...
	if (get_option('AWM_use_custom_menu_'.$awm_t)) { // if user wants a custom menu
		$awm_xml_out .=  AWM_create_existing_dynamic_menu($awm_t, "", $awm_ic, true);
	} else { // else use the other options
		if (get_option('AWM_include_home_'.$awm_t)) { // include home
