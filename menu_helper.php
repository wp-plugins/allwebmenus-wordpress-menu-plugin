...
function AWM_create_dynamic_menu__categories($awm_t, $awm_parentgroup, $awm_ic, $awm_isXML) {
	...
	$awm_tp = $wpdb->prefix;
	$awm_xml_out = "";
	$awm_isNew = ($wpdb->get_results("show tables like '{$awm_tp}term_taxonomy'")) > 0;
	...
	for ($awm_i=0; $awm_i<count($awm_pages); $awm_i++) {
		if ($awm_pages[$awm_i]->post_parent==$awm_group) {
			...
			if (AWM_page_has_kids($awm_pages[$awm_i]->post_parent, $awm_pages)) {
				if ($awm_isXML) {
					$awm_xml_out .= AWM_create_dynamic_menu__pages_step($awm_t, $awm_ic, "subMenu".$awm_depth, $awm_pages, $awm_depth+1, $awm_pages[$awm_i]->ID, $awm_recent, true);
				} else {
					echo "subMenu".$awm_depth."=item".$awm_depth.".newGroup('style=".$awm_m."_'+((wplevel+$awm_depth)==0?'sub_group_style':'sub_group_plus_style'));\n";
					$awm_ic = AWM_create_dynamic_menu__pages_step($awm_t, $awm_ic, "subMenu".$awm_depth, $awm_pages, $awm_depth+1, $awm_pages[$awm_i]->ID, $awm_recent, false);
				}
			}
			if ($awm_isXML) $awm_xml_out .= "</submenu></item>";
		}
