...
if (!isset($wp_session)) $wp_session = WP_Session::get_instance();
...
if ($_POST['theaction'] == "createnew") {
    ...
    $wp_session['message'] = '<div class="updated fade"><p><strong>' . $message . '</strong></p></div>';
    ob_end_clean();
    wp_redirect($_POST['ref']);
    exit;
    ...
    if ($awm_total_tabs) {
        ...
        $wp_session['message'] = '<div class="updated fade"><p><strong>There are no menus. You can create one using the appropriate button.</strong></p></div>';
        ob_end_clean();
        wp_redirect($_POST['ref']);
    }
    exit;
    ...
    $wp_session['message'] = '<div class="updated fade"><p><strong>Default Settings Loaded!</strong></p></div>';
    ob_end_clean();
    wp_redirect($_POST['ref']);
    exit;
    ...
    if (strpos(urldecode($_SERVER['HTTP_REFERER']), "wp-admin/options-general.php?page=allwebmenus-wordpress-menu-plugin/allwebmenus-wordpress-menu.php") === false) {
        echo "Access error! Please contact: support@likno.com";
        exit;
    }
